...
ipop_proteomics = ipop_proteomics[,-1]
ipop_proteomics[, c("SubjectID", "CollectionDate", "CL1", "CL2", "CL3", "CL4")] = list(NULL)
...
## Select samples that have proteomics
ipop_metadata = subset(ipop_metadata, SampleID %in% rownames(ipop_proteomics))
...
write.csv(subset.df, file = "ipop_data/ipop_infection_proteomics.csv")
...
write.csv(subset.df, file = "ipop_data/ipop_infection_proteomics_3.csv")
...
##################################
ipop_proteomics_selected = ipop_proteomics[dat$SampleID, ]

### May add psuedo count
ipop_proteomics_selected = ipop_proteomics_selected + 0.000000001

protein = list()
for(i in 1:ncol(ipop_proteomics_selected)){
  ## TODO: Check if the number matches by sample name
  print(i)
  protein[[i]] = cbind(dat, Count = ipop_proteomics_selected[,i], Feature = colnames(ipop_proteomics_selected)[i])
}
