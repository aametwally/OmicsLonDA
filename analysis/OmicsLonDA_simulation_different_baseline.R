...
evaluation_summary_earlydiff_norm = automateEval_OmicsLonDA(data = earlydiff_simulatedDataset_norm[1:2], n.perm = 100, points = points,
                                                           pvalue_threshold = 0.05, prefix = "evaluate_omicslonda_earlydiff_normalized_diffBaseline",
                                                           pattern = "earlydiff")
                                   ...
                                   pattern = "NNN")
{
  ...
  for(i in 1:length(data))
  {
    ...
    metalondaResults[[i]] = metalonda(Count = data[[1]]$Count, 
                                    ...
                                    Group = data[[1]]$Group,
                                    ID = data[[1]]$Subject, 
                                    n.perm = n.perm, fit.method = "nbinomial", points = points,
...
points = seq(1, 500, length.out = 500)
evaluation_summary_earlydiff_norm = automateEval_MetaLonDA(data = earlydiff_simulatedDataset_norm[1:2], n.perm = 100, points = points,
                                                           pvalue_threshold = 0.05, prefix = "evaluate_metalonda_earlydiff_normalized_diffBaseline",
                                                           pattern = "earlydiff")
