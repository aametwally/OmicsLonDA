...
########### Metabolomics     ##########
#######################################
ipop_metabolites = metbcr.df
rownames(ipop_metabolites) = ipop_metabolites$SampleID
ipop_metabolites = ipop_metabolites[,-1]
ipop_metabolites[, c("SubjectID", "CollectionDate", "CL1", "CL2", "CL3", "CL4")] = list(NULL)


### TODO: select annotated metabolites only
...
## Select samples that have metabolimics
ipop_metadata = subset(ipop_metadata, SampleID %in% rownames(ipop_metabolites))
...
write.csv(subset.df, file = "ipop_data/ipop_infection.csv")
...
write.csv(subset.df, file = "ipop_data/ipop_infection_3.csv")
...
##################################
### Select Metabolites samples 
##################################
ipop_metabolites_selected = ipop_metabolites[dat$SampleID, ]

### May add psuedo count
ipop_metabolites_selected = ipop_metabolites_selected + 0.000000001

metabolite = list()
for(i in 1:ncol(ipop_metabolites_selected)){
  ## TODO: Check if the number matches by sample name
  print(i)
  metabolite[[i]] = cbind(dat, Count = ipop_metabolites_selected[,i], Feature = colnames(ipop_metabolites_selected)[i])
}
